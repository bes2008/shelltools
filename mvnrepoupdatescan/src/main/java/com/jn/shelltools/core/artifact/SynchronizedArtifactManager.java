...
import com.jn.shelltools.core.artifact.repository.ArtifactRepository;
...
public class SynchronizedArtifactManager extends AbstractArtifactManager {
    ...
    public FileObject getArtifactFile(Artifact artifact) {
        try {
            ...
            if (!localFileObject.exists()) {
                if (Objs.isNotEmpty(srcArtifactRepositories)) {
                    Holder<FileObject> remoteFileObjHolder = new Holder<FileObject>();
